...
public class Advanced  {
	...
	public void init()
	{
		...
		faded_img_hor = null;
		try {
		    img_ver = ImageIO.read(new File("/Users/ahmedelehwany/Desktop/Battleship/ship_ver_2.png"));
		...
		//Carrier.setBounds(0, 0, 50, 250);
			/*faded_hor.setIcon(new ImageIcon(new ImageIcon(faded_img_hor).getImage().getScaledInstance(5*50, 50, Image.SCALE_DEFAULT)));
			faded_hor.setPreferredSize(new Dimension(5*50,50));
			//ship_img.setBounds(x*50, y*50, length*50, 50);
			faded_hor.setBounds(0,0, 5*50,50 );
					...
					else	//VERTICAL
					{
						...
						if(isValid2(new_x, new_y, 1, 5)==true && isValid2(x_mov, y_mov, 1, 5)==true)
						{
							...
							/*for(int i=0;i<5;i++)
							{
								//if(x_mov!=0 || y_mov!=0)
								//{
								if((x_mov+i+y_mov)%2==0)	{myboard[y_mov+i][x_mov].button.setBackground(Color.DARK_GRAY);}//reset even
							...
							faded_ver.setIcon(new ImageIcon(new ImageIcon(faded_img_ver).getImage().getScaledInstance(50, 5*50, Image.SCALE_DEFAULT)));
				...
				case 1:
					if (dir.getText()=="HORIZONTAL")
					{
						...
						if(isValid2(new_x, new_y, 0, 4)==true )
						{
							...
							/*for(int i=0;i<4;i++)
							{
								//if(x_mov!=0 || y_mov!=0)
								//{
								if((x_mov+i+y_mov)%2==0)	{myboard[y_mov][x_mov+i].button.setBackground(Color.DARK_GRAY);}//reset even
							...
							faded_hor.setIcon(new ImageIcon(new ImageIcon(faded_img_hor).getImage().getScaledInstance(4*50, 50, Image.SCALE_DEFAULT)));
					...
					else	//VERTICAL
					{
						...
						if(isValid2(new_x, new_y, 1, 4)==true )
						{
							...
							faded_ver.setIcon(new ImageIcon(new ImageIcon(faded_img_ver).getImage().getScaledInstance(50, 4*50, Image.SCALE_DEFAULT)));
				...
				case 2 : case 3:
					if (dir.getText()=="HORIZONTAL")
					{
						if(isValid2(new_x, new_y, 0, 3)==true )
						{
							...
							faded_hor.setIcon(new ImageIcon(new ImageIcon(faded_img_hor).getImage().getScaledInstance(3*50, 50, Image.SCALE_DEFAULT)));
					...
					else	//VERTICAL
					{
						...
						if(isValid2(new_x, new_y, 1, 3)==true )
						{
							...
							faded_ver.setIcon(new ImageIcon(new ImageIcon(faded_img_ver).getImage().getScaledInstance(50, 3*50, Image.SCALE_DEFAULT)));
				...
				case 4:
					if (dir.getText()=="HORIZONTAL")
					{
						if(isValid2(new_x, new_y, 0, 2)==true)
						{
							...
							faded_hor.setIcon(new ImageIcon(new ImageIcon(faded_img_hor).getImage().getScaledInstance(2*50, 50, Image.SCALE_DEFAULT)));
					...
					else	//VERTICAL
					{
						...
						if(isValid2(new_x, new_y, 1, 2)==true )
						{
							...
							faded_ver.setIcon(new ImageIcon(new ImageIcon(faded_img_ver).getImage().getScaledInstance(50, 2*50, Image.SCALE_DEFAULT)));
	...
	public boolean isValid2(int x, int y, int dir, int shiplen)
	{
		if(dir==0)//horizontal
		{
			for(int i=0;i<shiplen;i++)
			{
				if(y>9 || y<0 || x+i>9 || x+i<0 ||myboard[y][x+i].marked==true){System.out.println("not valid");return false;}
			}
		}
		else
		{
